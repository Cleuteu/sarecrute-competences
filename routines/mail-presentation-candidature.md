...
- **Mode anonyme.** Si le `Groupement` de la clinique est « Vetmint SA », ou si `Nom de la clinique` contient « Vetmint » ou « Vetplatform » (groupe suisse : Vetmint Fribourg et Vetplatform SA sont deux fiches du même client), la recruteuse présente les candidats **sans les nommer**. Le contrat signé avec ce groupe fait entrer un candidat dans la collaboration au moment où la clinique demande ses coordonnées ou ouvre son CV ; l'anonymat protège ce moment. Le mail se rédige quand même, selon la section « Le mode anonyme » de l'ÉTAPE 5. Même mode quand une note du candidat ou de la candidature demande l'anonymat (« à présenter en anonyme », « ne veut pas que son nom circule tant que… ») : dis alors dans la note ce qui l'a déclenché.
...
Quand l'ÉTAPE 3 a déclenché le mode anonyme, tout ce qui précède reste vrai — matière, ton, faiblesses acte par acte, conditions, réserves, questions — avec les écarts suivants, vérifiés sur les 12 mails anonymes envoyés à ce groupe entre février et septembre 2026 (exemples 4 et 5 du fichier de références).
