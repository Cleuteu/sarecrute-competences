...
- `Ville`, `Département`, `Pays` = descriptifs, depuis la source. `Département` s'écrit **numéro + nom en toutes lettres entre parenthèses** : « 29 (Finistère) », « 69 (Rhône) », « 2B (Haute-Corse) », « 974 (La Réunion) » — jamais le numéro seul, jamais le nom seul. Il se déduit de la ville quand c'est sans ambiguïté (Crozon → 29 (Finistère)), sinon vide ; un poste à l'étranger n'a pas de département. `Pays` quand il est dit ou évident (« Metz, France », « Lalín, Galice » → Espagne). Ces champs ne déclenchent aucun géocodage.
...
- `Période` = la période **normalisée**, jamais le verbatim (le verbatim, lui, va dans `Description`). Toujours la forme `repère - repère`, avec un tiret court entouré d'espaces et des repères pris dans ce vocabulaire fermé :
  - `Mars 2024` — la source donne le mois (mois en français, initiale en majuscule) ;
  - `Début 2024` / `Milieu 2024` / `Fin 2024` — la source donne une portion d'année ;
  - `Vers 2012` — l'incertitude dépasse l'année (« cinq ou six ans, jusqu'à fin 2017 ») ;
  - `Aujourd'hui` — en borne de fin seulement, quand `En cours` est coché.
  **Le repère se règle sur la précision de la SOURCE, pas sur celle de `Date début` / `Date fin`** : « fin 2017 » s'écrit `Fin 2017` et non `Décembre 2017` — le 01/12 n'est qu'une convention de saisie, l'afficher en clair serait mentir sur la précision. Un poste qui tient dans un seul mois répète son repère : `Janvier 2013 - Janvier 2013`. Exemples complets : `Octobre 2022 - Janvier 2024`, `Vers 2012 - Fin 2017`, `Fin 2017 - Début 2021`, `Mars 2026 - Aujourd'hui`. Si la source ne donne aucun repère utilisable pour cette ligne, laisse `Période` vide et dis-le dans `Description` — mieux vaut vide qu'inventé.
...
N'écris pas `Désignation`, `Durée`, `Dernière modification (contenu)` ni `Gelée` : ce sont des formules.

### C) Un poste, une ligne — et jamais deux fois le même

- **Un poste = une ligne.** Deux passages dans la même structure séparés par un changement de statut ou une coupure (salariée de 2021 à 2025, puis collaboratrice libérale depuis 2025) font deux lignes. Une série de remplacements courts dans la même clinique fait une ligne, dont la `Période` va du premier au dernier passage (`Milieu 2019 - Fin 2021`) et dont `Description` détaille les passages. Une mention groupée (« hivers 2017, 2018 et 2020 : prélèvements dans trois cabinets ») fait une ligne par cabinet nommé, dates vides, `Période` vide si on ne sait pas quel hiver revient à quel cabinet, et le verbatim conservé dans la `Description` de chacune.
...
Un CV donne des noms, des villes et des mois ; un entretien donne « j'ai travaillé cinq ou six ans en Espagne dans des référés, fin 2017 je suis venu en France, à Châtellerault, puis 2021 à Pontchâteau jusqu'à fin 2022 ». Ça fait trois lignes, sans `Structure`, avec `Type de poste` vide quand le statut n'est pas dit, des dates aux conventions du §B, le verbatim dans `Description` et des repères flous dans `Période` (`Vers 2012 - Fin 2017`, puis `Fin 2017 - Début 2021`, puis `Début 2021 - Fin 2022`). **Un parcours flou et vrai s'utilise ; un parcours précis et inventé fait perdre un rendez-vous.** L'oral ne donne pas les noms de cliniques : ne va pas les chercher ailleurs (rappel : aucune autre lecture n'est autorisée).
