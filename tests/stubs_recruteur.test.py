#!/usr/bin/env python3
"""Les cinq stubs recruteur téléchargent leur corps par `git clone` de `stable` d'abord (proxy GitHub
de la sandbox Cowork, seul chemin resté ouvert les 01/09, 10/09 et 11/09/2026), puis `raw` en second.
Un stub qui dériverait — un seul chemin, mauvais nom de compétence, repli sur une copie locale — ne se
verrait qu'au prochain incident réseau, chez une recruteuse.

    python3 tests/stubs_recruteur.test.py
"""
import re
import sys
...
INVARIANTS = (
    'git clone -q --depth 1 --branch stable "$REPO"',
...
def main() -> int:
    blocks = {}
    for s in SKILLS:
        p = ROOT / "plugins/sarecrute-recruteur/skills" / s / "SKILL.md"
    ...
    if diverge:
        ...
        return 1
    print("Bloc de téléchargement (git puis raw) identique dans les cinq stubs recruteur.")
    return 0
